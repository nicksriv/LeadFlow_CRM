...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/leads/:id/enrich-apollo", async (req, res) => {
    try {
      const { enrichLeadWithApollo } = await import("./apollo");
      const lead = await storage.getLead(req.params.id);
      ...
      const enrichmentResult = await enrichLeadWithApollo(lead);
      
      // Update lead with enriched data
      if (Object.keys(enrichmentResult.data).length > 0) {
        const updatedLead = await storage.updateLead(lead.id, enrichmentResult.data as any);
        
        // Store enrichment record
        await storage.createApolloEnrichment({
          leadId: lead.id,
          enrichmentData: enrichmentResult.apolloResponse,
          fieldsEnriched: enrichmentResult.enrichedFields,
          creditsUsed: 1,
          status: "success",
          errorMessage: null,
        });
        ...
        await storage.createActivity({
          leadId: lead.id,
          type: "lead_enriched",
          description: `Lead enriched via Apollo.io - ${enrichmentResult.enrichedFields.length} fields updated`,
          metadata: { fieldsEnriched: enrichmentResult.enrichedFields },
        });
  ...
  // Saleshandy integration routes
  app.post("/api/leads/:id/add-to-sequence", async (req, res) => {
    try {
      const { addLeadToSaleshandySequence } = await import("./saleshandy");
      const { sequenceStepId, sequenceName, verifyProspects, conflictAction } = req.body;
      
      if (!sequenceStepId) {
        return res.status(400).json({ error: "Sequence step ID is required" });
      }
      ...
      if (!lead.email) {
        return res.status(400).json({ error: "Lead must have an email to add to sequence" });
      }

      const result = await addLeadToSaleshandySequence(lead, sequenceStepId, {
        verifyProspects,
        conflictAction,
      });
      
      // Track sequence in database
      ...
      await storage.createActivity({
        leadId: lead.id,
        type: "sequence_added",
        description: `Added to Saleshandy sequence: ${sequenceName || "Unknown"}`,
        metadata: { sequenceId: sequenceStepId, sequenceName },
      });

      res.json({ 
        success: true, 
        message: "Lead added to sequence successfully",
        data: result.data 
      });
    } catch (error: any) {
      res.status(500).json({ error: error.message });
  ...
  app.post("/api/leads/bulk-add-to-sequence", async (req, res) => {
    try {
      const { bulkAddLeadsToSaleshandySequence } = await import("./saleshandy");
      const { leadIds, sequenceStepId, sequenceName, verifyProspects, conflictAction } = req.body;
      ...
      const validLeads = leads.filter((lead): lead is Lead => !!lead && !!lead.email);

      if (validLeads.length === 0) {
        return res.status(400).json({ error: "No valid leads with email addresses found" });
      }

      const result = await bulkAddLeadsToSaleshandySequence(validLeads, sequenceStepId, {
        verifyProspects,
        conflictAction,
      });
      
      // Track sequences in database
      await Promise.all(validLeads.map(lead => 
        storage.createSaleshandySequence({
          leadId: lead.id,
          sequenceId: sequenceStepId,
          sequenceName: sequenceName || "Unknown Sequence",
          stepId: sequenceStepId,
      ...
      res.json({ 
        success: true, 
        message: `${validLeads.length} leads added to sequence successfully`,
        addedCount: validLeads.length,
        data: result.data 
      });
    } catch (error: any) {
      res.status(500).json({ error: error.message });
